...
if __name__=='__main__':
    ...
    ecogiantsep = np.array([(192351.36/len(ecogiantmass[ecogiantmass>=M]))**(1/3.) for M in ecogiantmass])
    ecogiantsep = ecogiantsep*meansep0/np.median(ecogiantsep)

    # (b) make an interpolation function use this for RESOLVE-B  
    meansepinterp = interp1d(ecogiantmass, ecogiantsep, fill_value='extrapolate')
    ...
    plt.plot(ecogiantmass, ecogiantsep, 'k.', alpha=1, label=r'ECO Giant Galaxies ($logM* \geq 9.5$)')
    plt.plot(resblogmstar[resbgiantsel], resbgiantsep, 'r^', alpha=0.4, label=r'RESOLVE-B Giant Galaxies (interpolated, $logM* \geq 9.5$)')
    ...
    #rprojslope, rprojint = np.polyfit(uniqecogiantgrpn[keepcalsel], median_relprojdist, deg=1)
    #dvprojslope, dvprojint = np.polyfit(uniqecogiantgrpn[keepcalsel], median_relvel, deg=1)
    ...
    massbins=np.arange(9.5,13.5,0.5)
    binsel = np.where(np.logical_and(ecogdn>1, ecogdtotalmass<14))
    gdmedianrproj, massbinedges, jk = binned_statistic(ecogdtotalmass[binsel], ecogdrelprojdist[binsel], lambda x:np.nanpercentile(x,98), bins=massbins)
    gdmedianrelvel, jk, jk = binned_statistic(ecogdtotalmass[binsel], ecogdrelvel[binsel], lambda x: np.nanpercentile(x,98), bins=massbins)
    nansel = np.isnan(gdmedianrproj)
    massbinedges = massbinedges[:-1][~nansel]+0.25
    ...
    massbins2=np.arange(9.5,13.5,0.2)
    binsel2 = np.where(np.logical_and(resbana_gdn>1, resbana_gdtotalmass<14))
    ...
    resbcubicspline = interp1d(sortedmass, resbana_halomass[sortind])  
    ...
    plt.plot
    plt.xlabel("group-integrated stellar mass")
    ...
    ecodata.to_csv("ECOdata_G3catalog_stellarmass.csv")    
    ...
    resolvedata.to_csv("RESOLVEdata_G3catalog_stellarmass.csv")
