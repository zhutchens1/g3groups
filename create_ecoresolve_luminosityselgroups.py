...
if __name__=='__main__':
    ...
    ax1.set_ylabel(r"Relative Velocity to Group Center [km s$^{-1}$]")
    ...
    fig, (ax,ax1) = plt.subplots(ncols=2, figsize=doublecolsize)
    ax.plot(ecogdtotalmag[binsel], ecogdrelprojdist[binsel], '.', alpha=0.2, label='ECO Galaxies', rasterized=True, color='palegreen')
    ...
    ax.plot(tx, 1*decayexp(tx,*poptr), label=r'$R_{\rm proj,\,fit}^{\rm gi,\, dw}$', rasterized=True)
    ...
    ax.set_ylabel(r"Group Relative Projected Radius [$h^{-1}$ Mpc]")
    ax.legend(loc='best')
    ax.set_xlim(-25,-19)
    ax.set_ylim(0,0.8)
    ax.invert_xaxis()

    ax1.plot(ecogdtotalmag[binsel], ecogdrelvel[binsel], '.', alpha=0.2, label='ECO Galaxies', rasterized=True, color='palegreen')
    ...
    ax1.plot(tx, 4.5*decayexp(tx, *poptv), label=r'$4.5\Delta v_{\rm proj,\, fit}^{\rm gi,\, dw}$', rasterized=True)
    ax1.set_ylabel(r"Group Relative Velocity [km s$^{-1}]$")
    ...
    fig, ax = plt.subplots(figsize=(singlecolsize[0], 1.25*singlecolsize[1]))
    binv = np.arange(0.5,1200.5,3)
    ax.hist(fof.multiplicity_function(ecog3grp[ecog3grp!=-99.], return_by_galaxy=False), bins=binv, log=True, label='ECO Groups', histtype='step', linewidth=3, color='palegreen')
    ax.hist(fof.multiplicity_function(resbg3grp[resbg3grp!=-99.], return_by_galaxy=False), bins=binv, log=True, label='RESOLVE-B Groups', histtype='step', hatch='\\', color='k')
    ...
    ip = InsetPosition(ax, [0.4,0.25,0.5,0.35])
    ...
    ax2.hist(fof.multiplicity_function(ecoitassocid, return_by_galaxy=False), bins=binvd, log=True, label='ECO Groups', histtype='step', linewidth=3, color='palegreen')
    ax2.hist(fof.multiplicity_function(resbitassocid, return_by_galaxy=False), bins=binvd, log=True, label='RESOLVE-B Groups', histtype='step', hatch='\\', color='k')
    ax2.set_xlim(0,5)
    ax2.set_title('Dwarf-Only Groups', fontsize=9)
    plt.tight_layout()
